...
**Creative-system / spec reviews:** Creative PRISM is valid for generator specs, campaign systems, format frameworks, and creative operating models — not only finished assets. When the subject is a spec/system instead of a rendered artifact:
- treat section anchors like `[spec:format-policy]`, `[spec:quality-bar]`, `[spec:review-board-requirements]` as first-class creative citations;
- require at least one reviewer to test whether the spec defines **format-native narrative roles** rather than only new dimensions;
- require at least one reviewer to test for **brand anti-drift rails per format** (volt usage, footer/lockup legality, module compact states, prohibited simplifications);
- require at least one reviewer to test for **delight / emotional-premise coverage** so the system does not become merely operationally correct.
If the review target is a spec and no rendered artifact exists yet, do not penalize the absence of timestamps/frames; cite spec sections and current-source implementation files instead.
...
New brand slugs run in `accumulating` mode for the first 3 reviews. Memory is extracted and written but NOT injected into reviewers. After review #3, the orchestrator flags the file for human curation. Only after curation approval does `memory_status` flip to `active`.
...
- `review_count % 3 == 0` (orchestrator auto-triggers curation job)

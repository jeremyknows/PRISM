...
- `references/trust-boundary-grant-review.md` — Use when reviewing grants, descriptor allowlists, source registration, capability matrices, data-exposure policy surfaces, or any context/data exposure gate; fail-open grants, collapsed denial semantics, and arbitrary reportable reason text are blockers.
...
**Trust-boundary grant checkpoint:** If the review subject changes grants, descriptor allowlists, source registration, capability matrices, data-exposure policy surfaces, or any mechanism that decides which context/data can be exposed, read `references/trust-boundary-grant-review.md`. Passing tests and a mergeable PR are not enough: fail-open grants, collapsed denial semantics, or reportable arbitrary reason text are semantic blockers and should produce `NEEDS WORK` until fixed.

**OSS/package-boundary doc-truth checkpoint:** If the review subject is an extraction, portable-core/package split, or other OSS-boundary PR where public docs/roadmaps/master batons are part of the diff, treat wording/state drift as a first-class blocker. If docs still say the slice is "not extracted" / "next code lane" after the code landed, or break ladder/table structure such that operators get contradictory state, final verdict should be `NEEDS WORK` / `NEEDS_REMEDIATION` even when code/tests pass. Public/OSS wording hygiene is part of merge readiness, not post-merge polish.
...
**Multi-cycle roadmap/final-leg reviews:** When the operator asks for triple/multi-cycle PRISM over a roadmap, canary ladder, gate sequence, or promotion plan, read `references/multi-cycle-roadmap-review.md` before designing reviewer cycles. Prefer a small number of custom cycles with explicit synthesis over one PRISM per remaining phase; elevate simplicity findings and evidence-class labels so no-live/static proof is not mistaken for live/canary/fleet proof.
